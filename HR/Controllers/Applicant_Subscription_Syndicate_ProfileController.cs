...
namespace HR.Controllers
{
    public class Applicant_Subscription_Syndicate_ProfileController : BaseController
    {
        ...
        // GET: Applicant_Subscription_Syndicate_Profile
        public ActionResult Index(string id)
        {
            var ID = int.Parse(id);
            var new_model = dbcontext.Applicant_Subscription_Syndicate_Profile.Where(m => m.Applicant_Profile.ID == ID).ToList();
            ViewBag.idemp = id;
        ...
        public ActionResult Create(string id)
        {
            ...
            var model = dbcontext.Applicant_Subscription_Syndicate_Profile.ToList();
            var count = 0;
            if (model.Count() == 0)
            {
                count = 1;
        ...
        [HttpPost]
        public ActionResult Create(Applicant_Subscription_Syndicate_Profile model, string command)
        {
            try
            {
                ViewBag.Employee_Profile = dbcontext.Applicant_Profile.ToList().Select(m => new { Code = m.Code + "------[" + m.Name + ']', ID = m.ID });
                ...
                Applicant_Subscription_Syndicate_Profile record = new Applicant_Subscription_Syndicate_Profile();
                ...
                if (ModelState.IsValid)
                {
                    ...
                    var Employee_ProfileId = model.Employee_ProfileId;
                    ...
                    dbcontext.Applicant_Subscription_Syndicate_Profile.Add(record);
                    dbcontext.SaveChanges();
                    if (command == "Submit")
                    {
                        return RedirectToAction("edit", "Applicant_Profile", new { id = record.Employee_ProfileId });
                    }
        ...
        public ActionResult Edit(int id)
        {
            try
            {
                ...
                var record = dbcontext.Applicant_Subscription_Syndicate_Profile.FirstOrDefault(m => m.ID == id);
        ...
        [HttpPost]
        public ActionResult Edit(Applicant_Subscription_Syndicate_Profile model, string command)
        {
            try
            {
                ...
                var record = dbcontext.Applicant_Subscription_Syndicate_Profile.FirstOrDefault(m => m.ID == model.ID);
                if (model.Membership_type == Membership_type.Member)
                {
                    record.Membership_type = model.Membership_type;
        ...
        public ActionResult Deletemethod(int id)
        {
            ...
            try
            {
                dbcontext.Applicant_Subscription_Syndicate_Profile.Remove(record);
                dbcontext.SaveChanges();
