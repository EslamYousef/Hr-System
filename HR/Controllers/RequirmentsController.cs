...
{
    [Authorize(Roles = "Admin,Organization,OrganizationSetup,Requirments")]
    public class RequirmentsController : BaseController
    {
        // GET: Requirments
        ...
        public ActionResult Edit(int id)
        {
            try
            {
                var record = dbcontext.Requirments.FirstOrDefault(m => m.ID == id);
                if (record != null)
                { return View(record); }
                else
                {
        ...
        [HttpPost]
        public ActionResult Edit(Requirments model)
        {
            try
            {
                var record = dbcontext.Requirments.FirstOrDefault(m => m.ID == model.ID);
                record.Name = model.Name;
                record.Description = model.Description;
                record.Code = model.Code;
                dbcontext.SaveChanges();
                ...
                var get_result_check = HR.Controllers.check.check_alert("requirmetns", HR.Models.user.Action.edit, HR.Models.user.type_field.form);
                if (get_result_check != null)
                {
                    var inbox = new Models.user.Alert_inbox { send_from_user_id = User.Identity.Name, send_to_user_id = get_result_check.send_to_ID_user, title = get_result_check.Subject, Subject = get_result_check.Message };
                    if (get_result_check.until != null)
        ...
        public ActionResult Deletemethod(int id)
        {
            ...
            try
            {
                dbcontext.Requirments.Remove(record);
                dbcontext.SaveChanges();
                //=================================check for alert==================================

                var get_result_check = HR.Controllers.check.check_alert("requirmetns", HR.Models.user.Action.delete, HR.Models.user.type_field.form);
                if (get_result_check != null)
                {
                    var inbox = new Models.user.Alert_inbox { send_from_user_id = User.Identity.Name, send_to_user_id = get_result_check.send_to_ID_user, title = get_result_check.Subject, Subject = get_result_check.Message };
                    if (get_result_check.until != null)
