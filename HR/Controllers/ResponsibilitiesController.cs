...
{
    [Authorize(Roles = "Admin,Organization,OrganizationSetup,Responsibilities")]
    public class ResponsibilitiesController : BaseController
    {
        ...
        public ActionResult Edit(int id)
        {
            try
            {
                var record = dbcontext.Responsibilities.FirstOrDefault(m => m.ID == id);
                if (record != null)
                { return View(record); }
                else
                {
        ...
        [HttpPost]
        public ActionResult Edit(Responsibilities model)
        {
            try
            {
                var record = dbcontext.Responsibilities.FirstOrDefault(m => m.ID == model.ID);
                record.Name = model.Name;
                record.Description = model.Description;
                record.Code = model.Code;
                dbcontext.SaveChanges();
        ...
        public ActionResult Deletemethod(int id)
        {
            ...
            try
            {
                dbcontext.Responsibilities.Remove(record);
                dbcontext.SaveChanges();
                return RedirectToAction("index");
            }
            catch (DbUpdateException)
