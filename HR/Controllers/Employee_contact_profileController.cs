    ...
    public class Employee_contact_profileController : BaseController
    {
        ...
        public ActionResult Edit(string id)
        {
            try
            {
                ...
                ViewBag.Contactmethods = dbcontext.Contact_methods.ToList().Select(m => new { Code = m.Code + "------[" + m.Name + ']', ID = m.ID });
                var ID = int.Parse(id);
        var record = dbcontext.Employee_contact_profile.FirstOrDefault(m => m.Employee_ProfileId == ID.ToString());
                if (record != null)
                {
                    return View(record);
                }
        ...
        public ActionResult Edit(Employee_contact_profile model, string command)
        {
            try
            {
                ...
                var record = dbcontext.Employee_contact_profile.FirstOrDefault(m => m.Employee_ProfileId == model.ID.ToString());
