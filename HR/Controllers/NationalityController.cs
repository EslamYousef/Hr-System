...
{
    [Authorize(Roles = "Admin,Basic,BasicSetup,Nationality") ]
    public class NationalityController : BaseController
    {
        ...
        public ActionResult Create(Nationality model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var get_result_check = HR.Controllers.check.check_alert("nationality", HR.Models.user.Action.Create, HR.Models.user.type_field.form);
        ...
        public ActionResult Edit(Nationality model)
        {
            try
            {
                ...
                var get_result_check = HR.Controllers.check.check_alert("nationality", HR.Models.user.Action.edit, HR.Models.user.type_field.form);
                if (get_result_check != null)
                {
                    var inbox = new Models.user.Alert_inbox { send_from_user_id = User.Identity.Name, send_to_user_id = get_result_check.send_to_ID_user, title = get_result_check.Subject, Subject = get_result_check.Message };
                    if (get_result_check.until != null)
        ...
        public ActionResult Deletemethod(int id)
        {
            ...
            try
            {
                ...
                var get_result_check = HR.Controllers.check.check_alert("nationality", HR.Models.user.Action.delete, HR.Models.user.type_field.form);
                if (get_result_check != null)
                {
                    var inbox = new Models.user.Alert_inbox { send_from_user_id = User.Identity.Name, send_to_user_id = get_result_check.send_to_ID_user, title = get_result_check.Subject, Subject = get_result_check.Message };
                    if (get_result_check.until != null)
